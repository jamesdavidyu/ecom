...
func checkIfCartIsInStock(cartItems []types.CartItem, products map[int]types.Product) error {
	...
	for _, item := range cartItems {
		...
		if !ok {
			return fmt.Errorf("product %d is not available in the store, please refresh your car", item.ProductID)
		}

		if product.Quantity < item.Quantity {
			return fmt.Errorf("product %s is not available in the quantity requests", product.Name)
		}
	}
